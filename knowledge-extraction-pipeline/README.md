...
This pipeline talks to two local services over HTTP: **Docling Serve** (document conversion + chunking) and **Qdrant** (vector storage). Both run via Docker.
...
Add these to your `.env` (also needs `OPENAI_BASE_URL`, `OPENAI_API_KEY`, and `EMBD_MODEL` for embeddings):
...
Each step is a standalone script, run with `npx tsx`:

```
npx tsx ./knowledge-extraction-pipeline/1-extraction.ts   # convert a document
npx tsx ./knowledge-extraction-pipeline/2-chunking.ts     # hybrid-chunk it
npx tsx ./knowledge-extraction-pipeline/3-embedding.ts    # embed chunks + upsert to Qdrant
npx tsx ./knowledge-extraction-pipeline/4-search.ts       # vector search the collection
```
